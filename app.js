//for links//
let da = document.getElementById('da');
let scr = document.getElementById('sc');
let twit = document.getElementById('tw');
let ibis = document.getElementById('ib');
let dis = document.getElementById('di');
let git = document.getElementById('gi');
let cdp = document.getElementById('co');
let rob = document.getElementById('ro');

da.addEventListener('click', function(){
    window.open('https://www.deviantart.com/5h3rapr', 'target_blank')
});
scr.addEventListener('click', function(){
    window.open('https://scratch.mit.edu/users/AprillTest/', 'target_blank')
});
twit.addEventListener('click', function(){
    window.open('https://twitter.com/5h3rApr', 'target_blank')
});
ibis.addEventListener('click', function(){
    window.open('https://ibispaint.com/artist1/1651487144354119680/', 'target_blank')
});
dis.addEventListener('click', function(){
    alert('5h3rapr');
});
git.addEventListener('click', function(){
    window.open('https://github.com/SalmonSushiDev', 'target_blank')
});
cdp.addEventListener('click', function(){
    window.open('https://codepen.io/SalmonSushiDev', 'target_blank')
});
rob.addEventListener('click', function(){
    alert('Please ask on Discord :]');
});

//This was all made by me ^^
//Inspired by carrd.co! :D
